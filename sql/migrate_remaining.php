...
foreach ($creates as $sql) {
    ...
    echo $name . ': ';
    echo mysqli_query($conn, $sql) ? "OK\n" : mysqli_error($conn) . "\n";
}

// ALTERs restantes (ignorar duplicados)
$alters = [
    "ALTER TABLE `contratos` ADD COLUMN `usuario_acesso` VARCHAR(255) NULL, ADD COLUMN `senha_acesso` TEXT NULL",
    "ALTER TABLE `eventos` ADD COLUMN `link` VARCHAR(500) NULL, ADD COLUMN `usuario_acesso` VARCHAR(255) NULL, ADD COLUMN `senha_acesso` TEXT NULL, ADD COLUMN `data_inicio` DATETIME NULL, ADD COLUMN `data_fim` DATETIME NULL, ADD COLUMN `endereco` VARCHAR(500) NULL, ADD COLUMN `clima` VARCHAR(100) NULL, ADD COLUMN `id_integracao` VARCHAR(100) NULL, ADD COLUMN `ultima_sincronizacao` DATETIME NULL",
    "ALTER TABLE `pesquisas` ADD COLUMN `identificador_integracao` VARCHAR(100) NULL, ADD COLUMN `ultima_sincronizacao` DATETIME NULL",
];
foreach ($alters as $sql) {
    if (!mysqli_query($conn, $sql)) {
        echo 'ALTER skip: ' . mysqli_error($conn) . "\n";
    } else {
        echo "ALTER OK\n";
    }
}
